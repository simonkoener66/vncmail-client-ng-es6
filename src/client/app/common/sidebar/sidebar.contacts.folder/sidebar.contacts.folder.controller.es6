...
class SidebarContactFolderController {
  /* @ngInject */
  ...
  foldersAction(item){
    ...
    this.$state.go('contacts.otherContacts',{id: item.$.id, title: item.$.name});
  }

  getUserFolders(){
    let vm = this;
    vm.loading = true;
    vm.sidebarFoldersService._getContactFolders(function(folders){
      vm.loading = false;
      vm.folders = folders;
    });
  };
...
export default SidebarContactFolderController;
