...
class SidebarTaskFolderController {
  /* @ngInject */
  ...
  foldersAction(item){
    ...
    this.$state.go('tasks.tasks');
  }

  getUserFolders(){
    let vm = this;
    vm.loading = true;
    vm.sidebarFoldersService._getTaskFolders(function(folders){
      vm.loading = false;
      vm.folders = folders;
    });
  };
...
export default SidebarTaskFolderController;
